...
def axion_parameter_space(acc,fa_min,fa_max,ma_min,ma_max):
	...
	for i in range(len(m_ax)):	
		exlusion_limit2.append(1./(time_limit*years*sinvev*m_ax[i]))	
	exclusion_limits = exlusion_limit2[0] # DM: why is this first element returned?
	return(f_ax,m_ax,exlusion_limit2,exclusion_limits)
...
def exclusion_function(m_ax,f_a,X,Y,B,n2,l,m,exlusion_limit2,xtem,ytem,dytem,dxtem,plots=False,verbose=False):
	...
	for ii in range(len(m_ax)):
		...
		for j in range (len(axm2)):
			...
			for i in range(len(l)):
				...
				Zf[i]=(axm2[j]*X)**(4*l[i]+5)*(m[i]*Y-2*axm2[j]*((X + X*(1-Y**2)**0.5)))*B[i]*prdc*((exlusion_limit2[j]))**2/((X**2)/m[i]*delta_a)*Nbose	
				#########################################
				
				###############################
				# Matt's original expression
				#Zf[i]=(axm2[j]*X)**(4*l[i]+5)*(m[i]*Y-2*axm2[j]*((X + X*(1-Y**2)**0.5)))*B[i]*prdc*((exlusion_limit2[j]))**2*(10**78*c_0*(n2[i]**4/(X*axm2[j])**3)*((X/newton)/(10*solar_mass))**2*(f_a/planck)**2)/((X**2)/m[i]*delta_a)	
				#################################
		...
		for i in range (len(l)):
			...
			with warnings.catch_warnings():
				...
				cs[i] = plt.contour(X/(solar_mass*newton),Y,ZZf[0][i],[500],cmap=plt.cm.bone,alpha=1.0,linewidths = 2.5,zorder=2)		
